...
//   const [facility, setFacility] = useState(machine?.facility._id || '');
//   const [message, setMessage] = useState('');
//  console.log("machine",facility)
//   useEffect(() => {
...
const MachineForm = ({ onSubmit, machine, onClose }) => {
  ...
  const [machineName, setMachineName] = useState(machine?.machine_name || '');
  const [machineType, setMachineType] = useState(machine?.machine_type || '');
  const [facility, setFacility] = useState(machine?.facility?._id || ''); // Ensure we're getting the facility ID
  const [message, setMessage] = useState('');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Ensure that the facility ID is passed correctly in the payload
      await onSubmit({
        machine_name: machineName,
        machine_type: machineType,
        facility,
