...
//     const id=user?._id;
//     dispatch(updateUser({id,updateData})); // Dispatch update profile action
//     setShowUpdateModal(false); // Close modal after updating
//   };

//   const handleDeleteAccount = () => {
//     const id=user?._id;
//     dispatch(deleteUser({id})); // Dispatch delete account action
...
const ProfilePage = () => {
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    const updateData = { first_name: firstName, last_name: lastName, email, password };
    const id = user?._id;
