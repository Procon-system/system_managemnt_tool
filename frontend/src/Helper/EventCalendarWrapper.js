...
//   const [visibleResourceGroups, setVisibleResourceGroups] = useState({
//     assignedUsers: true,
//     resources: false,
//     clientAssets: false
//   });
...
//           headerToolbar: {
//             start: 'customToday,customPrev,customNext',
//             center: 'title',
...
//             customToday: {
//               text: 'Today',
//               click: () => calendarRef.current && calendarRef.current.setOption('date', new Date()),
//             },
//             customPrev: {
//               text: 'Previous Date',
//               click: () => calendarRef.current && calendarRef.current.prev(),
//             },
//             customNext: {
//               text: 'Next Date',
//               click: () => calendarRef.current && calendarRef.current.next(),
//             },
//             month: {
//               text: 'Month',
...
const EventCalendarWrapper = ({ events = [], onEventUpdate, onMultipleEventUpdate, calendarStartDate, onEventCreate, openForm, openCreateForm }) => {
  ...
  const assigned_resources = useMemo(() => {
    return groupedAssignedResources.filter(group => visibleResourceGroups[group.id]);
  }, [groupedAssignedResources, visibleResourceGroups]);
  const getTimezoneFromDate = (date) => {
    const timezoneOffsetMinutes = new Date(date).getTimezoneOffset(); // In minutes
    if (isNaN(timezoneOffsetMinutes)) {
      console.error('Invalid date for timezone calculation:', date);
    ...
    calendarRef.current = new Calendar({
      ...
      props: {
        ...
        options: {
          ...
          selectBackgroundColor: 'rgba(255, 68, 68, 0.2)',
          headerToolbar: {
            start: 'customToday customPrev title customNext',
            center: '',
            end: `${['resourceTimeGridDay', 'resourceTimelineDay', 'resourceTimeGridWeek', 'resourceTimelineMonth', 'resourceTimelineWeek'].includes(changedView) ? 'toggleResources ' : ''}year,month,week,day,list,resource,timeline`,
          },
          datesSet: (args) => {
            if (!isDateChangeAllowed.current) return;
            isDateChangeAllowed.current = false;
          ...
          customButtons: {
            toggleResources: {
              text: visibleResourceGroups.resources ? 'Hide Resources' : 'Show Resources',
              click: () => setVisibleResourceGroups(prev => ({...prev, resources: !prev.resources}))
            },
            // toggleAssets: {
            //   text: visibleResourceGroups.clientAssets ? '▼ Assets' : '▶ Assets',
            //   click: () => setVisibleResourceGroups(prev => ({...prev, clientAssets: !prev.clientAssets}))
            // },
            customToday: {
              text: 'Today',
              click: () => calendarRef.current && calendarRef.current.setOption('date', new Date()),
